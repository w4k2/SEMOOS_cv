...
directories = ["9lower", "9higher"]

n_datasets = 0
datasets = []
for dir_id, dir in enumerate(directories):
    DATASETS_DIR = os.path.join(os.path.realpath(os.path.dirname(__file__)), 'datasets/%s/' % dir)
...
experiments_paths = ["experiment5_cross_val_in_opt_9l", "experiment5_cross_val_in_opt_9h"]
...
IR = calc_imbalance_ratio(directories)
